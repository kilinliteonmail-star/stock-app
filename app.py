    ...
    with st.spinner(f"正在下載 {stock_symbol} 的歷史資料與計算扣抵值..."):
        try:
            ...
            if df.empty:
                ...
                # 準備圖表標記
                buy_markers = np.where(buy_signal, df['Low'] * 0.97, np.nan)
                sell_markers = np.where(sell_signal, df['High'] * 1.03, np.nan)

                # 截取最近 250 天來畫圖
                plot_df = df.iloc[-250:].copy()
                plot_buy = buy_markers[-250:]
                plot_sell = sell_markers[-250:]
                ...
                # 關鍵修改：讓 mplfinance 回傳圖表物件 (fig)，這樣 Streamlit 才能畫出來
                fig, axlist = mpf.plot(
